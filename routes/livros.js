...
router.post('/add', function(req, res) {
  ...
  db.query(cmd, [isbn, titulo, sinopse, ano, preco, estoque, 
                 conservacao, media, avaliacoes, editora], function(erro, resultado){
  if (erro){
    res.send(erro);
  } else {
    var id_livro = resultado.insertId; 
    // Insere em tb_autor_livro 
    if (Array.isArray(autores)) { 
      autores.forEach(id_autor => { 
        cmd = `INSERT INTO tb_autor_livro (id_autor, id_livro) VALUES (?, ?);`; 
        db.query(cmd, [id_autor, id_livro], function(erro){ 
          if (erro) 
            console.error("Erro ao inserir relação:", erro); 
        }); 
      }); 
    }
  }
